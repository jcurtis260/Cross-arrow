...
export function getLineDirection(line: Line): Direction {
  ...
  if (path && path.length > 1) {
    ...
    const arrowhead = path[path.length - 1];

    if (penultimate.x !== arrowhead.x) {
      return arrowhead.x > penultimate.x ? 'right' : 'left';
    }

    return arrowhead.y > penultimate.y ? 'down' : 'up';
  }
...
export function canLineSlide(line: Line, direction: Direction, grid: Grid): boolean {
  ...
  const finalPoint = line.path?.at(-1);
  const arrowhead = finalPoint
    ? { x: finalPoint.x, y: finalPoint.y, occupied: true, lineId: line.id }
    : getLeadingCell(cells, direction);

  // Connected paths leave as a snake: the arrowhead needs a free lane and
  // the body follows its existing route. Checking every body cell would
  // incorrectly reject valid bent-path exits.
  for (const pathCell of getForwardPath(arrowhead, direction, grid.size)) {
